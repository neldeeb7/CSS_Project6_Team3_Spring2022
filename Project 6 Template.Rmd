...
```{r}
# Generate empty data frame to hold stats from random simulations
simulations_df5 = data.frame(matrix(ncol=3, nrow=1000))
...
for (sim in 1:r5) {
  ...
  match_gen_att <- matchit(formula = formula(features_form),
                           data = ypsps_dropna_droplater, # fullest data while omitting NAs and irrelevant columns 
                           method = "genetic", distance = "glm", link = "logit", estimand = "ATT", replace = TRUE, ratio = 2)

  # save model in df
  simul_models5[[sim]] <- match_gen_att
